...
var START_FEN = 'rnbxqksbnr/pppppppppp/10/10/10/10/PPPPPPPPPP/RNBXQKSBNR'
  var START_POSITION = fenToObj(START_FEN)
    ...
    Object.keys(START_POSITION).forEach(function(square) {
      if (!validSquare(square)) {
        console.warn('Invalid square in position:', square, 'with value', START_POSITION[square]);
      }
      if (!validPieceCode(START_POSITION[square])) {
        console.warn('Invalid piece code in position:', square, 'with value', START_POSITION[square]);
      }
    });//addition
  ...
  function validSquare (square) {
    return isString(square) && square.search(/^[a-j][1-8]$/) !== -1
  }
  ...
  function fenToPieceCode(piece) {
    if (piece.search(/[1-9]/) !== -1) return null; // Ignore numbers
  ...
  fen = fen.replace(/ .+$/, '')
  fen = expandFenEmptySquares(fen) // <-- ADD THIS LINE
  var rows = fen.split('/')
  var position = {}

  var currentRow = 8
  for (var i = 0; i < 8; i++) {
    if (currentRow < 1) break; // Defensive: don't go below row 1
    ...
    for (var j = 0; j < row.length; j++) {
      if (row[j].search(/[1-9]/) !== -1) {
        ...
        var code = fenToPieceCode(row[j]);
        if (code && validSquare(square)) {//addition
  ...
  function squeezeFenEmptySquares (fen) {
    return fen.replace(/1111111111/g, '10')
  ...
  function expandFenEmptySquares (fen) {
    return fen.replace(/10/g, '1111111111')
  ...
  function constructor (containerElOrString, config) {
    ...
    var startRow = 8, endRow = 1, step = -1;
    if (orientation === 'black') {
      alpha.reverse();
      startRow = 1;
      endRow = 8;
    ...
    for (var row = startRow; (step === -1 ? row >= endRow : row <= endRow); row += step) {
      html += '<div class="{row}">';
      for (var j = 0; j < COLUMNS.length; j++) {
        var square = alpha[j] + row;
        console.log(square); // Add this line
    // ...
    ...
    function dropDraggedPieceOnSquare (square) {
      console.log('Dropping on square:', square); // Add this line
      removeSquareHighlights()
    ...
    function stopDraggedPiece (location) {
      console.log('Stop dragged piece at:', location); // Add this line
